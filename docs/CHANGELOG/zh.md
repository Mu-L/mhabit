...
- 功能：习惯分组功能，包含分组管理页面、分组模式切换、可折叠分组和跨组拖拽排序，目前位于实验性功能开关后（#617）
- 更新法语、希伯来语和意大利语翻译，感谢 Weblate 贡献者（#616）

## 1.25.7+172

- 功能：同步 Android 动画缩放比例与 Flutter 设置（#608）
- 功能：更新日志首次打开弹窗和横幅集成（#610）
- 功能：支持从 Loop Habit Tracker (uhabit) CSV 导出导入习惯和记录（#612）
- 功能：实现自适应内容面板并优化弹窗本地化（#613）
- 功能：在习惯往返同步中保留未知同步字段以实现前向兼容（#614）
- 修复：更新日志标题中的预发布后缀版本匹配问题
- 修复：将基于正则的测试版标题匹配替换为 AST 前缀匹配，提升可靠性（#615）
- 更新希伯来语翻译，感谢 Omer I.S. 在 Weblate 上的贡献（#611）

## 1.25.3+168

- 更新希伯来语翻译，感谢 Omer I.S. 在 Weblate 上的贡献（#589）
- 更新土耳其语翻译，感谢 Oğuz Ersen 在 Weblate 上的贡献（#589）
- 新增自动化商店提交流水线（#590, #591, #592, #593）

## 1.25.1+164

- 功能：新增每个习惯的自定义颜色支持，包含内置色板、取色器和最近使用颜色历史（#580）
...
- 修复返回上一层时 Habit Detail 页面重复刷新的问题（#565）
- 更新通用加载失败提示的希伯来语文案，并刷新翻译生成产物（#570）
- 稳定图标字体生成流程并刷新发布所需的生成图标资源（#574）

## 1.24.4+160-pre

- 改进习惯提醒在应用启动、重启和日期变更场景下的刷新与修复处理（#569）
- 更新通用加载失败提示的希伯来语翻译（#570）

## 1.24.3+157-pre

- 重构 Provider 架构与页面依赖组装，并统一页面加载失败/重试处理，补充相关本地化文案与测试覆盖（#563）
- 修复 Android predictive back 返回时 Habit Detail 页面重复刷新的问题（#565）

## 1.24.2+156

- 维护性版本：修复影响 F-Droid 分发的构建问题（#543, #544）
- 本版本不包含用户可感知的功能或行为变更

## 1.24.1+154

- 新增巴斯克语，并更新阿拉伯语、法语、希伯来语、土耳其语和繁体中文翻译，同时补充语言元数据与贡献者信息（#523, #529）
- 更新商店元数据、品牌色、摘要、安装文案，以及 f_store 的注册资源（#519, #525, #529, #534）
- 升级 Flutter 到 3.41.9，并更新依赖、生成文件、lockfile、发布校验与新工具链运行时集成（#540）
- 修复 App Sync server 的 iter type 处理与主题颜色问题（#540）
- 更新 README 与 source builder wiki 文档（#525, #540）
...
- 更新阿拉伯语、法语、希伯来语、土耳其语和繁体中文翻译，感谢 Weblate 贡献者的贡献（#529）
- 更新 Android、iOS、macOS、Windows 本地化元数据、贡献者及 README/安装指南文案（#519 #525 #529）
- 为 f_store flavor 添加 Android ADI 注册资源
- 更新 Flathub 元数据主品牌色和摘要（#534）
...
- 启用搜索筛选器拖动并支持页面滚动（#504）
- 更新翻译文件，刷新 ARB 格式并清理空元数据
- 更新德语翻译，感谢 TheKos 在 Weblate 上的贡献（#503）
- 将 Darwin Xcode 版本提升到 26（#500）
- 为 iOS/macOS 添加元数据和更新日志校验（#501）
- 更新 Donors.md

## 1.23.6+145

- 更新简体中文翻译，感谢 ADAIBLOG 在 Weblate 上的贡献（#490）
- 更新俄语翻译，感谢 Rudoy Artyom 在 Weblate 上的贡献（#490）
- 优化提醒频率本地化，改用按周/月/天的模板（#491）
- 补充简体中文中的目标天数单位文本（#491）
- 修复：今日习惯在切换记录状态后重置展开状态，避免界面残留（#492）
- 修复：修复初始加载时空状态图片闪烁的问题（#494）
- 移除未使用的空视频元数据

## 1.23.3+142

- 功能：Android 14+ 启用预测返回手势（#483）
- 功能：系统主题使用平台特定颜色方案（#487）
- 修复：iPadOS 上导出弹窗位置异常
- 升级 Flutter 3.35.7 / Dart，并迁移相关 API (#479)
- Android：升级 AGP 到 8.12.3，同时调整 Kotlin/Gradle (#479)
- iOS/macOS：工程同步到新工具链 (#479)
- 依赖更新 (#479)
- 代码格式化和清理 (#479)
- 更新反馈邮件地址
...
- 新增匈牙利语翻译，感谢 peterczegledy 在 GitHub 上的贡献 (#462)
- 更新乌克兰语翻译，感谢 Максим Горпиніч 在 Weblate 上的贡献 (#459)
- 更新越南语翻译，感谢 Loc Huynh 在 Weblate 上的贡献 (#467)
- 修复：通过安全处理错误，防止加载过程卡住
- 修复：同步开始时触发 `RefreshIndicator`
- 修复：Play Store 版本添加本地应用名称（#472）
- 更新 README.md (#463)

## 1.22.6+133

- 功能：添加自定义主题颜色 (#447)
- 添加 Windows 提醒支持并启动每日提醒 (#450)
- 添加 Linux 提醒支持并启用重复提醒 (#453)
- WebDAV 同步可以递归创建缺失的根文件夹并显示解码后的路径 (#454)
- 更新乌克兰语翻译，感谢 Максим Горпиніч 在 Weblate 的贡献 (#446)
- 添加乌克兰语 F-Droid/Google 描述翻译，感谢 Максим Горпініч 在 Weblate 的贡献 (#446)
- 为在 Microsoft Store 上发布准备 (#444)
- 为 MSIX 添加多语言支持 (#448)
- 添加 `Fastlane:android:update_play_store` 任务，可使用可选参数

## 1.22.2+129

- 为第三方许可条目添加图标 (#431)
- 空搜索结果显示特殊图片 (#436)
- 更新法语翻译，感谢 Massine Y 在 Weblate 上的贡献 (#428)
- 更新乌克兰语翻译，感谢 Максим Горпиніч 在 Weblate 上的贡献 (#428)
- 修复：将尚未开始的习惯排除在今日列表之外 (#435)
- 修复：调整今日页面部分 UI (#434)
- 修复：iPadOS 26 顶部双击 (#437)
- 修复：linux-arm64 下 CJK 和 emoji 无法正常显示（Flatpak 沙箱中 CJK 渲染错误除外） (#438)
- 添加 Android / iOS / macOS 的 fastlane (#425, #426)
- 迁移 `DateTime.now()` 到 `AppClock().now()` (#430)
- 更新 README
...
- 更新希伯来语翻译，感谢 sudo-py-dev 在 Weblate 的贡献 (#421)
- 添加日语翻译，感谢我的朋友 Shirleycrow 的贡献 (#423)

**🧹 其他**

- 修复：使用 `MediaQuery.sizeOf(context)` 进行响应式布局尺寸计算 (#418)
- 修复：应用代码审查反馈的 UI 逻辑清理和修复 (#418)
- 修复：确保日期变化时“今日”页面正确刷新 (#418)

## 1.21.1+120

**🌐 本地化**

- 更新西班牙语翻译，感谢 Angel 在 Weblate 上的贡献
...
- 改进 `habit_display` 的焦点、滚动和键盘交互 (#409)
- 添加全局 `UnfocusOnTap` 包装类 (#410)

**🌐 本地化**

- 更新阿拉伯语翻译，感谢 Abdelbasset Jabrane 在 Weblate 上的贡献
- 新增葡萄牙语（巴西）F-Droid 描述翻译，感谢 Jonathan Soares 在 Weblate 上的贡献

**🧹 其他**

- 修复点击空白处无法自动取消焦点的问题
- 修复 Android 上日历滚动到边缘时无法收起的问题 (#411)

## 1.20.1+115

**🌐 本地化**

- 更新乌克兰语翻译，感谢 maksim2005UKR 在 Weblate 的贡献。
- 更新俄语 F-Droid 描述翻译，感谢匿名贡献者在 Weblate 的支持。

**🧹 其他**

- 修复：阻止 Dispatcher 在布局/尺寸变化时被重复使用
- 重构：采用新的 ViewModel 调用 (#405)
- 重构：明确 Widget 所绑定 ViewModel 边界 (#400)
- 重构：简化 Provider 初始化 (#404)
- 重构：Habit 相关模块架构清理 (#397, #398, #401, #403)
